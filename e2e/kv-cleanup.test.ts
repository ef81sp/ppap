  ...
  try {
    await t.step(
      ...
      async () => {
        ...
        assertEquals(
          oldRoomCheck.value?.id,
          oldRoomId,
          '古いルームが正しく保存されていない'
        );
        assertEquals(
          newRoomCheck.value?.id,
          newRoomId,
          '新しいルームが正しく保存されていない'
        );
        ...
        const oldSocketAfter = await kv.get([
          'socket_instances',
          'old-user-token',
        ]);
        ...
        assertEquals(
          oldUserRoomAfter.value,
          null,
          '古いユーザールーム関連が削除されていない'
        );
        assertEquals(
          oldRoomUpdatesAfter.value,
          null,
          '古いルーム更新情報が削除されていない'
        );
        assertEquals(
          oldSocketAfter.value,
          null,
          '古いソケットインスタンスが削除されていない'
        );

        // 孤立したソケットインスタンスが削除されたことを確認
        const orphanedSocketAfter = await kv.get([
          'socket_instances',
          'orphaned-socket',
        ]);
        assertEquals(
          orphanedSocketAfter.value,
          null,
          '孤立したソケットインスタンスが削除されていない'
        );
        ...
        const newSocketAfter = await kv.get([
          'socket_instances',
          'new-user-token',
        ]);
        ...
        assertNotEquals(
          newUserRoomAfter.value,
          null,
          '新しいユーザールーム関連が誤って削除された'
        );
        assertNotEquals(
          newRoomUpdatesAfter.value,
          null,
          '新しいルーム更新情報が誤って削除された'
        );
        assertNotEquals(
          newSocketAfter.value,
          null,
          '新しいソケットインスタンスが誤って削除された'
        );
      }
    );
  } finally {
