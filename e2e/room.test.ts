...
import { cleanupKVStore, createPage, setupBrowser, startServer, stopServer } from "./helpers.ts"
...
Deno.test("ルーム機能のテスト", async (t) => {
  ...
  await t.step("別のユーザーがルームに参加できること", async () => {
    ...
    try {
      ...
      const $voteOption = await queries.getByRole($newDocument, "button", { name: "1" })
      ...
      const $articles = await queries.getAllByRole($articlesDocument, "article")
      ...
      // 検証: 参加者数は2人（ホストとゲスト）
      assertEquals($articles.length, 2, "参加者数は2人（ホストとゲスト）であるべきです")
  ...
  await t.step("観戦者モードに切り替えられること", async () => {
    ...
    try {
      ...
      $document = await getDocument(page)
      const $checkbox = await queries.getByLabelText($document, "I'm an audience.")
      ...
      const audienceTextExists = await queries.queryByText($document, "Audience") !== null
      assertEquals(audienceTextExists, true, "Audienceテキストが画面上に表示されているか")
    } finally {
      // 必ずページとコンテキストをクローズ
      await page.close()
      await _context.close()
  ...
  await t.step("投票をクリアできること", async () => {
    ...
    try {
      ...
      // クリアボタンをクリック
      const $clearButton = await queries.getByRole($document, "button", { name: "clear" })
