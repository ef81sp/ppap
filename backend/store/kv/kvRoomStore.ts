...
export class KVRoomStore implements RoomStore {
  ...
  private async notifyLocalUsersAboutRoomChange(room: Room): Promise<void> {
    ...
    console.log(
      `Checking room ${room.id.slice(
        0,
        6
      )}... participants for instance ${instanceId.slice(0, 6)}...`
    );
        ...
        if (userInstanceId === instanceId) {
          ...
          if (socket && socket.readyState === WebSocket.OPEN) {
            ...
            console.log(
              `Notified local user ${participant.token.slice(
                0,
                6
              )}... about room ${room.id.slice(0, 6)}... change via watch`
            );
          } else {
            console.log(
              `User ${participant.token.slice(
                0,
                6
              )}... has no active socket or socket is not open`
            );

            // ソケットがない場合、KVストアから該当ユーザーの情報をクリーンアップ
            if (!socket) {
              console.log(
                `Cleaning up stale socket instance reference for ${participant.token.slice(
                  0,
                  6
                )}...`
              );
          ...
          console.log(
            `User ${participant.token.slice(
              0,
              6
            )}... belongs to instance ${userInstanceId.slice(
              0,
              6
            )}..., not this instance`
          );
