...
// 部屋の作成
export const createRoom = async (userToken: UserToken, userName: string): Promise<Room> => {
...
// 回答のクリア
export const clearAnswer = async (roomId: RoomId): Promise<Room | undefined> => {
  return await getStoreManager().getRoomStore().clearAnswer(roomId)
}

// 部屋からの退出
export const exitRoom = async (userToken: UserToken): Promise<Room | undefined> => {
...
// WebSocket追加
export const addSocket = async (userToken: UserToken, socket: WebSocket): Promise<void> => {
